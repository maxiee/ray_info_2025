...
from rayinfo_backend.collectors.base import BaseCollector, ParameterizedCollector, RawEvent
...
class MockSimpleCollector(BaseCollector):
    ...
    async def fetch(self):
        """模拟采集数据"""
        yield RawEvent(
            post_id=f"test-{int(time.time())}",
            source=self.name,
            raw={"test": "data"}
        )


class MockParameterizedCollector(ParameterizedCollector):
    """测试用参数化采集器"""
    ...
    async def fetch(self, param: str):
        """模拟参数化采集"""
        yield RawEvent(
            post_id=f"test-{param}-{int(time.time())}",
            source=self.name,
            raw={"query": param, "test": "data"}
        )
...
class TestCollectorStateManager(unittest.TestCase):
    ...
    def test_first_run_detection(self):
        ...
        # 应该立即执行
        should_run = self.state_manager.should_run_immediately("test.collector", None, 300)
    ...
    def test_immediate_run_logic(self):
        ...
        # 应该立即执行（超过间隔时间）
        should_run = self.state_manager.should_run_immediately(collector_name, None, interval)
        self.assertTrue(should_run)
    ...
    def test_next_run_time_calculation(self):
        ...
        # 首次运行，应该立即执行
        next_time = self.state_manager.calculate_next_run_time(collector_name, None, interval)
...
class TestSchedulerIntegration(unittest.TestCase):
    ...
    async def test_parameterized_execution_with_state(self):
        ...
        # 验证参数化状态已更新
        stats = self.scheduler_adapter.state_manager.get_collector_stats(collector.name, param)
        self.assertIsNotNone(stats)
        self.assertEqual(stats["execution_count"], 1)
...
class TestCompleteWorkflow(unittest.TestCase):
    ...
    def test_multiple_collectors_independence(self):
        ...
        for i, (name, param) in enumerate(collectors):
            ...
            self.assertAlmostEqual(stats["last_execution_time"], expected_time, delta=1.0)
