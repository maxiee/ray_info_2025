...
class ParameterizedJobFactory(JobFactory):
    ...
    def create_job(
        ...
        async def param_job_wrapper():
            logger.debug(
                "执行参数化采集器任务 collector=%s param=%s",
                collector.name, 
                param
            )
            events = []
...
class ParameterizedJobStrategy(JobScheduleStrategy):
    ...
    def schedule_job(
        ...
        try:
            ...
            logger.error(
                "获取参数化任务列表失败 collector=%s error=%s", 
                collector.name, e
            )
            return []
        ...
        for param, interval in param_jobs:
            ...
            logger.info(
                ...
                job_id, param, interval, instance_id
            )
...
class StrategyRegistry:
    ...
    def get_strategy(self, collector: BaseCollector) -> JobScheduleStrategy:
        ...
        if hasattr(collector, 'list_param_jobs') and callable(getattr(collector, 'list_param_jobs')):
