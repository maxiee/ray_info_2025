...
class InstanceIDManager:
        ...
        with self._lock:
            ...
            while attempt < max_attempts:
                if attempt == 0:
                    ...
                    suffix_hash = hashlib.sha256(suffix_input.encode("utf-8")).hexdigest()
            ...
            timestamp_id = hashlib.sha256(f"{base_hash}:{time.time()}".encode()).hexdigest()[:8]
            return timestamp_id
    ...
    def update_instance_stats(self, instance_id: str, success: bool = True, error_msg: str | None = None) -> bool:
        """更新实例运行统计
