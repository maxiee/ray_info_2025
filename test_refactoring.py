...
from rayinfo_backend.collectors.base import RawEvent, SimpleCollector, ParameterizedCollector
from rayinfo_backend.scheduling.scheduler import SchedulerAdapter
from rayinfo_backend.pipelines.base import DedupStage, PersistStage, Pipeline
...
class TestParameterizedCollector(ParameterizedCollector):
    ...
    async def fetch(self, param=None):
        ...
        for i in range(2):
            yield RawEvent(
                source=self.name,
                raw={"post_id": f"{param}_{i}", "query": param, "content": f"Content for {param} - {i}"},
                debug=True  # 测试模式
            )
...
def test_pipeline_optimization():
    ...
    try:
        ...
        events = [
            ...
            RawEvent("test.source", {"post_id": "1", "content": "Content 1 duplicate"}),  # 重复
            RawEvent("test.source", {"post_id": "3", "content": "Content 3"}),
            RawEvent("test.source", {"post_id": "2", "content": "Content 2 duplicate"}),  # 重复
        ]
        ...
        print(f"✓ 去重测试：输入 {len(events)} 个事件，输出 {len(unique_events)} 个事件")
        ...
        # 测试管道组合
        pipeline = Pipeline([
            DedupStage(max_size=100),
            PersistStage()  # 测试阶段，只打印
        ])
...
def test_enhanced_pipeline():
    ...
    try:
        ...
        events = [
            ...
            RawEvent("test.source", {"url": "http://example.com", "content": "URL content"}),
            RawEvent("test.source", {"url": "http://example.com", "content": "URL duplicate"}),
        ]
        ...
        # 测试指标收集
        metrics = dedup_stage.get_metrics()
        print(f"✓ 指标收集: 处理数 {metrics['processed_count']}，去重数 {metrics['duplicates_found']}")
