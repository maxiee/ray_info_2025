...
class TaskConsumerRegistry:
    ...
    def register(self, source: BaseTaskConsumer) -> None:
        if source.name in self.sources:
            raise ValueError(f"TaskConsumer with name '{source.name}' already registered")
...
class TestTaskConsumer(BaseTaskConsumer):
    ...
    async def consume(self, task: Task) -> None:
        ...
        logger.info(f"[{self.name}] 完成任务 {task.uuid[:8]} (耗时 {end_time - start_time:.2f}s)")
...
async def test_time_ordering():
    ...
    try:
        ...
        task3 = consumer.produce({
            "message": "task3",
            "schedule_at": now + timedelta(seconds=0.3)
        })
        task1 = consumer.produce({
            "message": "task1", 
            "schedule_at": now + timedelta(seconds=0.1)
        })
        task2 = consumer.produce({
            "message": "task2",
            "schedule_at": now + timedelta(seconds=0.2)
        })
...
async def test_early_task_insertion():
    ...
    try:
        ...
        late_task = consumer.produce({
            "message": "late_task",
            "schedule_at": now + timedelta(seconds=0.5)
        })
